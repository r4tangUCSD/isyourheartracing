...
export function createWholeGraph(caseId) {
    ...
    let existingSVG = d3.select("#chart").select("svg");
    if (existingSVG.empty()) {
        existingSVG = d3.select("#chart").append("svg");
    }
    existingSVG.selectAll("*").remove(); // Clear contents but keep the SVG
